...
public class DbMigrationManager {
    ...
    /**
     * Runs special auto-generated DbPatch0 on empty database.
     */
    public void runDbPatch0() throws Exception {
        log.info("Applying migration DbPatch0 (initializing empty database)...");
        txManager.runInTransaction(new TxRunnable() {
            @Override
            public void run(Connection conn) throws Exception {
                val migration = ApplicationContextFactory.createAutowiredBean(DbPatch0.class);
                migration.execute();
            }
        });
    }
     ...
     */
    public List<DbPatch> runAll(Context ctx, List<Class<? extends DbPatch>> migrations) throws Exception {
        if (!ctx.isDbInitialized()) {
            // Might have been initialized by DbPatch0.
            ctx.appliedMigrationNames = queryAppliedMigrationNames();
            ctx.oldDbVersion = queryOldDbVersion();
        }
        ...
        if (ctx.appliedMigrationNames == null) {
            ...
            txManager.runInTransaction(new TxRunnable() {
                ...
                public void run(Connection conn) throws Exception {
                    ...
                    // TODO Remove in WFE 5. Migrations already applied must be inserted by DbPatch0.
        ...
        val appliedNow = new ArrayList<DbPatch>();
        long whenAllStarted = System.currentTimeMillis();
        for (Class<? extends DbPatch> c : migrations) {
            if (UnsupportedPatch.class.isAssignableFrom(c) || EmptyPatch.class.isAssignableFrom(c)) {
                continue;
            }
            val name = c.getSimpleName();
