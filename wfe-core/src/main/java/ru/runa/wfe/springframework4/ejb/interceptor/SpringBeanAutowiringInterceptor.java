package ru.runa.wfe.springframework4.ejb.interceptor;
...
import ru.runa.wfe.springframework4.context.access.ContextSingletonBeanFactoryLocator;

/**
 * EJB3-compliant interceptor class that injects Spring beans into fields and methods which are annotated with {@code @Autowired}. Performs injection
 * after construction as well as after activation of a passivated bean.
 *
 * <p>
 * To be applied through an {@code @Interceptors} annotation in the EJB Session Bean or Message-Driven Bean class, or through an
 * {@code interceptor-binding} XML element in the EJB deployment descriptor.
 *
 * <p>
 * Delegates to Spring's {@link AutowiredAnnotationBeanPostProcessor} underneath, allowing for customization of its specific settings through
 * overriding the {@link #configureBeanPostProcessor} template method.
 *
 * <p>
 * The actual BeanFactory to obtain Spring beans from is determined by the {@link #getBeanFactory} template method. The default implementation obtains
 * the Spring {@link ContextSingletonBeanFactoryLocator}, initialized from the default resource location
 * <strong>classpath*:beanRefContext.xml</strong>, and obtains the single ApplicationContext defined there.
 *
 * <p>
 * <b>NOTE: If you have more than one shared ApplicationContext definition available in your EJB class loader, you need to override the
 * {@link #getBeanFactoryLocatorKey} method and provide a specific locator key for each autowired EJB.</b> Alternatively, override the
 * {@link #getBeanFactory} template method and obtain the target factory explicitly.
 *
 * <p>
 * <b>WARNING: Do not define the same bean as Spring-managed bean and as EJB3 session bean in the same deployment unit.</b> In particular, be careful
 * when using the {@code <context:component-scan>} feature in combination with the deployment of Spring-based EJB3 session beans: Make sure that the
 * EJB3 session beans are <i>not</i> autodetected as Spring-managed beans as well, using appropriate package restrictions.
 *
...
public class SpringBeanAutowiringInterceptor {
    ...
    /*
     * We're keeping the BeanFactoryReference per target object in order to allow for using a shared interceptor instance on pooled target beans. This
     * is not strictly necessary for EJB3 Session Beans and Message-Driven Beans, where interceptor instances get created per target bean instance. It
     * simply protects against future usage of the interceptor in a shared scenario.
     */
     ...
     * 
     * @param processor
     *            the AutowiredAnnotationBeanPostProcessor to configure
     * @param target
    ...
    /**
     * Determine the BeanFactoryLocator key to use. This typically indicates the bean name of the ApplicationContext definition in
     * <strong>classpath*:beanRefContext.xml</strong> resource files.
     * <p>
     * The default is {@code null}, indicating the single ApplicationContext defined in the locator. This must be overridden if more than one shared
     * ApplicationContext definition is available.
     * 
     * @param target
     *            the target bean to autowire
     * @return the BeanFactoryLocator key to use (or {@code null} for referring to the single ApplicationContext defined in the locator)
     */
