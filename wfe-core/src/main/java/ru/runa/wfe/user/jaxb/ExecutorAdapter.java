...
public class ExecutorAdapter extends XmlAdapter<WfExecutor, Executor> {
    ...
    @Override
    public Executor unmarshal(WfExecutor wfExecutor) {
        return (Executor) ApplicationContextFactory.getTransactionalExecutor().executeWithResult(() -> {
            return ApplicationContextFactory.getExecutorDao().getExecutor(wfExecutor.getId());
        });
    }
}
