...
public class FrozenProcessInParallelGatewaySeeker implements FrozenProcessSeeker {
    ...
    public List<WfFrozenToken> seek(Integer timeThreshold, Map<FrozenProcessFilter, String> filters) {
        ...
        List<CurrentToken> gatewayTokens;
        int pageNumber = 0;
        do {
            ...
            for (CurrentToken token : gatewayTokens) {
                ParsedProcessDefinition processDefinition = processDefinitionLoader.getDefinition(token.getProcess());
                try {
    ...
    private StateInfo findStateInfo(ParallelGateway gateway, Long processId) {
        ...
        List<CurrentToken> tokens = currentTokenDao.findByProcessIdAndParentIsNull(processId);
