    ...
    <v-tabs vertical>
      ...
      <v-tab-item>
        <v-card flat width="500" class="mx-auto my-10">
          ...
          />
          <v-btn small color="primary" class="mt-5">Применить</v-btn>
        </v-card>
      </v-tab-item>
      <v-tab-item>
...
export default {
  components: { ProfileUserCard, ProfilePasswordForm },
  data() {
    return {
      loading: false,
      user: null,
          ...
          new SwaggerClient({
              ...
              const data = client.apis['auth-controller'].tokenUsingPOST({
                  ...
                  client.then((client: any) => {
                      const data = client.apis['profile-api-controller']
                              .getProfileUsingPOST().then((data: any) => {
