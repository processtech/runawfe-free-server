 ...
 * 
 * @struts:action path="/lockProcessDefinition" name="idNameForm" validate="false" input = "/WEB-INF/wf/manage_process.jsp"
 ...
 */
public class LockProcessDefinitionAction extends ActionBase {
    public static final String ACTION_PATH = "/lockProcessDefinition";
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse responce) {
        ...
        try {
            Delegates.getDefinitionService().lockProcessDefinition(getLoggedUser(request), form.getName(), false);
        } catch (Exception e) {
            addError(request, e);
            return Commons.forward(mapping.findForward(Resources.FORWARD_FAILURE), IdForm.ID_INPUT_NAME, form.getId());
        }
        addMessage(request, new ActionMessage(MessagesProcesses.DEFINITION_LOCKED.getKey()));
