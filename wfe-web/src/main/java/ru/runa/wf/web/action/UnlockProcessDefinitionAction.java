 ...
 * 
 * @struts:action path="/unlockProcessDefinition" name="idNameForm" validate="false" input = "/WEB-INF/wf/manage_process.jsp"
...
public class UnlockProcessDefinitionAction extends ActionBase {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse responce) {
        ...
        try {
            Delegates.getDefinitionService().unlockProcessDefinition(getLoggedUser(request), form.getName());
        } catch (Exception e) {
            addError(request, e);
